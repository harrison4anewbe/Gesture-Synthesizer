...
def gesture_synthesizer(input_word, frame, video_label, current_thread=None):
    video_paths = {
        "joy": "./mov/joy.mov",
        "sadness": "./mov/sadness.mov",
        "anger": "./mov/anger.mov",
        "fear": "./mov/fear.mov",
        "neutral": "./mov/neutral.mov",
    }
...
def generate_gesture(input_word, frame, video_label, predictor, current_thread=None):
    emotions = ["joy", "sadness", "anger", "fear", "neutral"]
    message = input_word.lower()
    prediction = predictor.predict(message)
    print(prediction)
    if prediction in emotions:
        gesture_synthesizer(prediction, frame, video_label, current_thread)
    else:
...
def main():
    ...
    input_field.pack()
    create_tooltip(input_field, "Enter your text. e.g., How are you?")

    input_button = tk.Button(
        frame,
        text="Generate Gesture",
        command=lambda: generate_gesture(
            input_field.get(), frame, video_label, predictor, current_thread
        ),
    )
    input_button.pack()
